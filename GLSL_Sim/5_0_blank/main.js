...
class SimonDevGLSLCourse {
  ...
  raf_() {
    requestAnimationFrame((t) => {
      if (this.previousRAF_ === null) {
        this.previousRAF_ = t;      // setting previous request animation frame to current time
      }

      this.step_(t - this.previousRAF_); // call the step everytime raf is called will lead to accumulation error
  ...
  step_(timeElapsed) {
    const timeElapsedS = timeElapsed * 0.001; // set
    this.totalTime_ += timeElapsedS; // total time in project setup is incremented by time elapsed
